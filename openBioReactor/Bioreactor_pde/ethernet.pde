...
byte server[] = {
  10,0,0,29}; // local NTP server 
const int NTP_PACKET_SIZE= 48; // NTP time stamp is in the first 48 bytes of the message
byte packetBuffer[NTP_PACKET_SIZE]; //buffer to hold incoming and outgoing packets 
unsigned long epoch; // Unix time (starts on Jan 1 1970)

// WebClient constructor; on port 80
Client client(server, 80); //the server is on the local network
...
//  if(DEBUG)Serial.println(sizeof(jsonCommand));
  ...
  while(client.connect() && i<500) // 500ms timeout
  {
    i++;
    delay(1);
  }
