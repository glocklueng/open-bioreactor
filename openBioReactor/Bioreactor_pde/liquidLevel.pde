...
float liquidHeightGlobal; // gloabl variable for export; in [inch] it easier to check because the eTape (sensor) is in [inch]
float liquidHeightInterval;
float liquidHeightVolume;
...
float liquidLevelGetInverval()
{
  return liquidHeightInterval;
  
}

float liquidLevelGetVolume()
{
  return liquidHeightVolume;
